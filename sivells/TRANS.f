...
9     FC(6,J)=ONE-FC(6,J)/SUM
      ...
      LR=NN
      RC=RTO-ONE
      WRITE (6,12) ITLE,RC,AWOP,AWOPP,AWOPPP
      DO 10 J=1,NN
      Y=DFLOAT(J-1)/(NN-1)
      YY=Y*Y
      Y4=YY**2
     ...
     1ALF*(YY-ONE)/(3-IE))/RTO
      ...
      WRITE (6,13) Y,UY,VY,WY,YM
10    IF (MOD(J,10) .EQ. 0) WRITE (6,14)
      XX1=CUBIC(CWOPPP/SIX,AWOPP/TWO,AWOP,WO-ONE)
      XXI=CUBIC(AWOPPP/SIX,AWOPP/TWO,AWOP,WO-W)
      WRITE (6,15) XX1,XXI,W,CWOPPP,TK
      WRITE (6,16)
      PX=AXN+1.D-1
      DO 11 J=1,11
      X=.1D+0*(J-1)
      ...
      WRITE (6,18) AXN,W,AWP,AWPP,AMN,AMP,AMPP
11    WRITE (6,17) X,XW,XWP,XWPP,XM,XMP,XMPP
      RETURN
C
