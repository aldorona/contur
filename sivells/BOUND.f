...
4     DRX(J)=WALTAN(J)
      ...
      IF (LD .GE. 0) WRITE (6,80) ITLE,PPS,T0
5     IF (ALPH .GT. ZRO) GO TO 6
      ALPHA=ZRO
      IF ((LD .GE. 0) .OR. (PPQ .EQ. ZRO)) WRITE (6,71)
      GO TO 7
6     ALPHA=ALPH
      IF ((LD .GE. 0) .OR. (PPQ .EQ. ZRO)) WRITE (6,70)
7     IF (IR .EQ. 2) GO TO 13
      IF (ALF .EQ. ONE) GO TO 8
      IF ((LD .GE. 0) .OR. (PPQ .EQ. ZRO)) WRITE (6,75)
      GO TO 9
8     IF ((LD .GE. 0) .OR. (PPQ .EQ. ZRO)) WRITE (6,72)
9     IF (IR) 10,11,12
10    IF ((LD .GE. 0) .OR. (PPQ .EQ. ZRO)) WRITE (6,74)
      GO TO 14
11    IF ((LD .GE. 0) .OR. (PPQ .EQ. ZRO)) WRITE (6,73)
      GO TO 14
12    IF ((LD .GE. 0) .OR. (PPQ .EQ. ZRO)) WRITE (6,76)
      GO TO 14
13    IF ((LD .GE. 0) .OR. (PPQ .EQ. ZRO)) WRITE (6,77)
...
37    DELTA=DOTR*TH
      ...
      RTHI=RTHX
      GO TO 30
38    WRITE (6,88) RTHX,REO,FRD,TH,DELTA,DOT
      RETURN
...
46    THA=TH
      ...
      IF (J .EQ. 1 .AND. LD .GE. 0) WRITE (6,82)
...
48    IF (LD.LT.0) GO TO 56
      ...
      IEO=REO+HALF
      ITHX=RTHX+HALF
      WRITE (6,83) J,TW,TE,TAW,TP,IEO,ITHX,FRD,CFIK,CFK,CFSK,H,HI,FMY,DT
     1HK,TH,DELTA,DELST
...
55    WRITE (6,86) S(J),DSU,THU,CTH,HU,H,CH,XN
56    CONTINUE
      RW(1)=RCO(1)
...
57    DRX(J)=WALTAN(J)+DAX(J)
      ...
      IF (KBL.LE.KAT) WRITE (6,85) XBL,DLST,THBL,HBL,XNBL,DLTBL,REOFT,RE
     1TH,RETHG,REDL,REDLG
      ...
      RETHG=DLOG10(RETH)
      REDLG=DLOG10(REDL)
      IF (KBL.GT.KAT) WRITE (6,85) S(KAT),CDS(KAT),CTH,HBL,XN,DELTA,REOF
...
58    CONTINUE
      ...
      WRITE (6,78) ITLE,XBIN,XCIN,SF
      PPQ=ZRO
      WRITE (6,67) RC,ETAD,AMACH,BMACH,CMACH,EMACH,MC,AH
      IF (TWT.NE.ZRO) GO TO 59
      WRITE (6,81) PPS,T0
      GO TO 5
59    WRITE (6,79) PPS,T0,TWT,TWAT,TAIR
      GO TO 5
...
62    SL(J+1)=SL(J-1)+S1*SCV(J-1)+S2*SCV(J)+S3*SCV(J+1)
      XST=ZRO
      WRITE (6,68) LS,DK
      WRITE (6,69) (K,S(K),SL(K),DLA(K),RCO(K),WALTAN(K),SK(K),DAX(K),DR
     1X(K),WMN(K),DMDX(K),SPR(K),BTA(K),K=1,KAT)
      ...
      WRITE (6,89) XBL,SBL,DLAB,RCOB,GMA,GMB,GMC,GMD
      GO TO 64
63    WRITE (6,68) LY,DD
      WRITE (6,69) (K,S(K),FS(K),DLA(K),RCO(K),WALTAN(K),SD(K),DAX(K),DR
     1X(K),WMN(K),DMDX(K),SPR(K),BTA(K),K=1,KAT)
      IF (KBL.GT.KAT) GO TO 64
      ...
      RCOB=GMA*RCO(KBL-3)+GMB*RCO(KBL-2)+GMC*RCO(KBL-1)+GMD*RCO(KBL)
      YBL=RCOB-DLAB
      WRITE (6,84) XBL,YBL,DLAB,RCOB,GMA,GMB,GMC,GMD
64    WRITE (6,87) XST,YST,DD2,DR2,RCV
      S(1)=XST
      RCO(1)=YST
      DRX(1)=ZRO
      ...
      IF (LV .GT. 0) GO TO 3
65    CONTINUE
      IF (J .EQ. 1) WRITE (6,90) IPQ,QFUNW,TWT
      RETURN
C
66    FORMAT (6E10.3,4I5) 
67    FORMAT (1H ,4H RC=,F11.6,3X,5HETAD=F8.4,4H DEG,3X,6HAMACH=F10.7,3X
     1,6HBMACH=F10.7,3X,6HCMACH=F10.7,3X,6HEMACH=F10.7,3X,A4,2HH=F11.7/)
68    FORMAT (1H ,7X,9HSTA(IN)    ,A4,40H(IN)    DELR(IN)    R(IN)    DY
     1/DX   ,A8,50H     DA/DX     DR/DX     MACH NO.    DM/DX    PE/PO,7
     2X,4HBETA/)
...
78    FORMAT (1H1,3A4,39HNOZZLE CONTOUR, RADIAL FLOW ENDS AT STA,F12.7,2
     15H, TEST CONE BEGINS AT STA,F12.7,16H, SCALE FACTOR =,F13.8/)
79    FORMAT (1H ,1X,15HSTAG. PRESSURE=F5.0,24H PSI, STAG. TEMPERATURE=,
     1F5.0,21H DEG R, THROAT TEMP.=F5.0,19H DEG R, WALL TEMP.=F4.0,24H D
     2EG R, THROAT HT COEF.=F8.5//)
80    FORMAT (1H1,3A4,49HBOUNDARY LAYER CALCULATIONS, STAGNATION PRESSUR
     1E=F5.0,28HPSI, STAGNATION TEMPERATURE=,F5.0,27H DEG R, N BASED ON 
     2RE,DELTA //)
81    FORMAT (1H  ,5X,15HSTAG. PRESSURE=F5.0,24H PSI STAG. TEMPERATURE=,
     1F5.0,34H DEG R ADIABATIC WALL TEMPERATURE//)
82    FORMAT (1H  ,5X,38HTW    TE    TAW    TP    RE/IN    RTHI,        
     14X,3HFRD,5X,4HKCF1,4X,3HKCF,5X,4HRCFS,5X,1HH,6X,2HHI,5X,38HFMY    
     2 KTHP THETA-1  DELTA  DELTA*-1 /)
83    FORMAT (1H ,I3,2F6.1,F7.1,F6.1,I9,I7,4F8.5,F8.4,F7.4,2F8.5,F9.6 ,
     1F7.4,F9.6 )
84    FORMAT (1H ,3HSTA,2F11.6,2F11.7,7X,27HINTERPOLATION COEFFICIENTS,,
     1 F12.8,1H,,F11.8,1H,,F11.8,1H,,F12.8/)
85    FORMAT (1H0,5H   X=,F7.3,11H,   DELTA*=,F10.7,10H,  THETA=,F9.7,6
     1H,   H=,F10.6,6H,   N=,F10.7,10H,   DELTA=,F11.7,10H,   RE/FT=,F11
     2.0//35X,9HRE,THETA=,F9.0,8H,  LOG=,F8.5,1H,,16X,9HRE,DELTA=,F11.0
     3,8H,   LOG=,F8.5)
86    FORMAT (1H ,3X,2HX=,F7.3,8H,   DSU=,F8.5,8H,   THU=,F9.7,8H,   CTH
     1=,F9.7,7H,   HU=,F10.6,6H,   H=,F10.6,7H,   CH=,F10.6,6H,   N=,F8.
     25)
87    FORMAT (1H  ,3HSTA,F11.6,9H     Y*=,F11.7,14H,     D2A/DX2=,F12.9,
     114H,    D2R/DX2=,F12.9,16H,     VISCID RC=,F14.8 )
88    FORMAT (1H ,'RTHX=',1PE12.5,', REO=',E12.5,', FRO=',0PF8.5,', TH='
     1,F8.5,', DELTA=',F8.5,', DOT=',F9.5 )
89    FORMAT (1H ,3HSTA,2F11.6,2F11.7,7X,27HINTERPOLATION COEFFICIENTS,,
     1F12.8,1H,,F11.8,1H,,F11.8,1H,,F12.8 /)  
90    FORMAT (1H0,' ITERATION',I4,',    QFUN =',F8.5,',   THROAT TEMP  =
     1',F6.1 / )
      END