...
C
     ...
     1OP,QM,WE,CBET,XE,ETA,EPSI,BPSI,XO,YO,RRC,SDO,XB,XC,AH,PP,SE,TYE,XA
      ...
      DATA ZRO/0.0D+0/,ONE/1.D+0/,TWO/2.D+0/,OCT/8HCURVATUR/
...
C
     ...
     1FORM='FORMATTED',ACTION='READ',BLANK='NULL')
      OPEN(UNIT=6, FILE='data.txt', STATUS='OLD')
1     READ (1,30,END=24) ITLE,JD
      IF (ITLE(1) .EQ. L4) GO TO 24
      IE=1+JD
      QT=ONE/(1+IE)
C
      READ (1,28) GAM,AR,ZO,RO,VISC,VISM,SFDA,XBL
...
6     SLONG=S(KING)-S(1)
      IPP=0
      WRITE (6,25) ITLE,SLONG
      WRITE (6,31) LA,L2,DCA,DC3,DCC,DCB,LB
      IF (JB .GT. 0) GO TO 7
      WRITE (6,26) XST,FS(1),WALTAN(1),ZRO,SD(1)
      XMAX=SLONG+XST
      YMAX=FS(KING)
      TMAX=WALTAN(KING)
      GO TO 8
7     WRITE (6,26) XST,RCO(1),DRX(1),ZRO,DR2
      XMAX=S(KAT)-S(1)+XST
      YMAX=RCO(KAT)
      TMAX=DRX(KAT)
...
13    XX=X-XST+S(1)
      ...
      WANG=CONV*AJ(K)
      CURV=YYPP/DCOS(AJ(K))
      WRITE (6,26) X,YY,YYP,WANG,CURV
      GO TO 18
14    YY=YY-S(1)+XST
      XJ(K)=YY
      WANG=CONV*DACOS(YYP)
      WRITE (6,26) X,YY,YYP,WANG
      GO TO 18
16    WANG=CONV*DATAN(YYP)
      ...
      WALTAN(K+1)=YYP
      SD(K+1)=YYPP
17    IF (XEND.GT.ZRO.OR.JB.LT.0) WRITE (6,26) X,YY,YYP,WANG,YYPP
      IF (XEND.EQ.ZRO.AND.JB.GE.0) WRITE (6,26) X,YY,YYP,WANG,YYPP,DY
18    IF (MOD(K,10) .EQ. 0) WRITE (6,29)
...
19    CONTINUE
      ...
      IPP= IPP+1
      WRITE (6,25) ITLE, SLONG
      WRITE (6,31) LA,L2,DCA,DC3,DCC
      WRITE (6,26) XST,RCO(1),DRX(1),ZRO,DR2
      GO TO 11
20    IF ((IPP .GT. 0) .OR. (JX .LT. 0)) GO TO 1
      ...
      WRITE (6,29)
      WRITE (6,31) L3,L1,DC6,DC3
      WRITE (6,26) XST,XST,ONE,ZRO
      GO TO 11
21    IF (JB) 1,2,22
...
23    DPX(K+1)=-GAM*YY*YYP*SPR(K+1)/TTR(K+1)
      ...
      IF (JB .GT. 10) GO TO 1
      WRITE (6,25) ITLE, SLONG
      WRITE (6,31) L1,L2,DC4
      WRITE (6,27) (S(K),RCO(K),DRX(K),K=1,KAT)
      GO TO 1
24    STOP
C
